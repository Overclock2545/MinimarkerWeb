...
<div class="container mt-4">
    <h2 class="mb-4 text-center" style="color: #a855f7;">👥 Lista de Usuarios</h2>
                        ...
                        <td>
                            <a href="{{ route('admin.usuarios.editar', $usuario->id) }}" 
                               class="btn btn-outline-primary btn-sm mb-1">
                                {{ Auth::user()->rol === 'admin' ? '✏️ Editar' : '👁 Ver datos' }}
                            </a>

                            <a href="{{ route('admin.usuarios.carrito', $usuario->id) }}" 
                               class="btn btn-outline-info btn-sm mb-1">🛒 Ver Carrito</a>

                            <a href="{{ route('admin.usuarios.pedidos', $usuario->id) }}" 
                               class="btn btn-outline-success btn-sm mb-1">📦 Ver Pedidos</a>

                            @if(Auth::user()->rol === 'admin')
                            <!-- Botón que activa el modal -->
                            <button type="button" class="btn btn-outline-danger btn-sm" 
                                    data-bs-toggle="modal" 
                                    data-bs-target="#confirmarEliminarModal{{ $usuario->id }}">
                                🗑 Eliminar
                            </button>
