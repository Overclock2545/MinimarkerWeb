    ...
    @isset($producto)
        ...
        <div style="display: flex; gap: 40px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 300px;">
                <img
                    src="{{ $producto->imagen ?? 'https://via.placeholder.com/350' }}"
                    alt="{{ $producto->nombre }}"
                ...
                >
                    ...
                    <button
                        type="submit"
                        style="margin-top: 15px; background-color: #222; color: white; padding: 10px; border: none; border-radius: 4px;"
                    >
                        Agregar al carrito 🛒
                    </button>
                </form>
                ...
                <button
                    style="margin-top: 10px; background-color: #fbbacb; border: none; padding: 10px; border-radius: 4px;"
                >
                    Agregar a favoritos ⭐
                </button>
            </div>
              ...
              <small>
                Categoría: {{ $product->categoria->nombre ?? 'Sin categoría' }}
                        ...
                        @auth
                            <form
                                method="POST"
                                action="{{ route('carrito.agregar', $product->id) }}"
                                class="card-footer"
                            >
                                @csrf
